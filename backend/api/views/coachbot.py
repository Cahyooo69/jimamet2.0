...
def coachbot_chat(request):
    ...
    try:
        ...
        # Coba menggunakan Gemini API secara langsung
        if api_key:
            ...
            try:
                ...
                fallback_error = f"Gemini error: {str(e)}"
        else:
            fallback_error = "GEMINI_API_KEY tidak diatur."

        # Fallback menggunakan OpenRouter jika gagal
        if not ai_reply:
            openrouter_api_key = os.environ.get("OPENROUTER_API_KEY")
            if not openrouter_api_key:
                return Response(
                    {"error": f"{fallback_error} | OpenRouter juga tidak bisa digunakan karena OPENROUTER_API_KEY belum diatur."}, status=500
                )

            try:
                headers = {"Authorization": f"Bearer {openrouter_api_key}", "Content-Type": "application/json"}
                data = {
                    # Gunakan model di openrouter, misalnya gemini atau model ringan lain
                ...
                return Response({"error": f"{fallback_error} | OpenRouter error: {str(e)}"}, status=500)
        ...
        return Response({"reply": ai_reply, "needs_consultation": needs_consultation}, status=200)
